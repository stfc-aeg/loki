  ...
  namespace eval VLAB {
   # -----------------------------------------------------------------------------------------------------------------------------------------
    ...
    proc hw_reload_prope_file_device {{fname ""}} {
      ...
      if {[file exists ${TE::PREBUILT_HW_PATH}/${shortdir_int}/${TE::PB_FILENAME}.ltx]} {
        set_property PROBES.FILE ${TE::PREBUILT_HW_PATH}/${shortdir_int}/${TE::PB_FILENAME}.ltx [get_hw_devices $hw_fpga_name]
        TE::UTILS::te_msg TE_PR-30 INFO "New Probes file is set: ${TE::PREBUILT_HW_PATH}/${shortdir_int}/${TE::PB_FILENAME}.ltx"
        
      } else {
        TE::UTILS::te_msg TE_PR-31 INFO "Probes file (${TE::PREBUILT_HW_PATH}/${shortdir_int}/${TE::PB_FILENAME}.ltx)  doesn't exist."
        set_property PROBES.FILE "" [get_hw_devices $hw_fpga_name]
      }
    }  
    # -----------------------------------------------------------------------------------------------------------------------------------------
