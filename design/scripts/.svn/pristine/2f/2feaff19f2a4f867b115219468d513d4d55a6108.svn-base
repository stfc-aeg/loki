...
namespace eval ::TE {
  namespace eval UTILS {
    ...
    proc setinfo_to_block_design_tcl {datalist mod_file} {
      ...
      set data [linsert $data[set data {}] 0 "set scripts_vivado_version [version -short ]"]
      set data [linsert $data[set data {}] 0 "################################################################"]
   ...
   # puts "Test:$typ | [file exists ${baseFolder}/${osPath}/${shortDir}/${appPath}]|${baseFolder}/${osPath}/${shortDir}/${appPath} "
    ...
    proc copy_sw_files {} {
      ...
      foreach app $applications {
        ...
        if {![string match  "executable.elf" ${filenameonly}] && ![string match */qemu/* ${app}] && ![string match "u-boot.elf" ${filenameonly}]  && ![string match "bl31.elf" ${filenameonly}]} {
